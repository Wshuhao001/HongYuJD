 ...
 */
// 代码修改_start_derek20150129admin_goods  www.hongyuvip.com
function get_brand_list($t = false)
// 代码修改_end_derek20150129admin_goods  www.hongyuvip.com
{
...
/* 代码增加_虚拟团购_END  www.hongyuvip.com */
...
/**
 *  清除手机指定后缀的模板缓存或编译文件
 *  wei2 增加 start by www.hongyuvip.com
 * @access  public
 * @param  bool       $is_cache  是否清除缓存还是清出编译文件
 * @param  string     $ext       需要删除的文件名，不包含后缀
 *
...
/* wei2 增加 end by www.hongyuvip.com */
/**
 * 清除模版编译文件
 *
 * @access  public
 ...
 * @return  array
 */
/* 修改 by www.hongyuvip.com 增加一个参数 */
function get_package_goods($package_id, $package_attr_id='')
{
	...
	if ($package_attr_id)
	{
	 ...
	 // 下面SQL语句增加两个字段，注意逗号 , g.goods_thumb, g.shop_price   By  www.hongyuvip.com
    $sql = "SELECT pg.goods_id, g.goods_name, pg.goods_number, p.goods_attr, p.product_number, p.product_id, g.goods_thumb, g.shop_price 
            ...
            WHERE pg.package_id = '$package_id' $sql_package_attr_id "; //有修改 by www.hongyuvip.com 注意最后那个 $sql_package_attr_id
    if ($package_id == 0)
    {
...
function Recordkeyword($word_www_68ecshop_com, $items = 0, $searchengine = 'ecshop')
{
	...
	if($r)
	{
		$items = intval($items) ;   //www.hongyuvip.com
	...
	if(array_search(0,$supplier_ids) !== false){
		if ($position == 1){
			if ($_CFG['sms_order_placed'] == '1' && $_CFG['sms_shop_mobile'] != '')
		    {
                ...
                $content1 = array($content,"{\"orderSn\":\"$order_sn\"}",$_CFG['sms_sign']);
