...
if ($_REQUEST['step'] == 'add_to_cart')
{
	...
	//www.hongyuvip.com start add 2015-3-26
	$time_xg_now=gmtime();
	...
	//www.hongyuvip.com end add 2015-3-26
	...
	$result['goods_id'] = $goods->goods_id;
	$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";//添加 www.hongyuvip.com
...
/* 代码增加_end  By www.hongyuvip.com */
    ...
    else
    {
        ...
        elseif (!empty($_POST['act']) && $_POST['act'] == 'signup')
        {
            ...
            {
                /* 用户注册成功 */
				/* 四合一修改 start by www.hongyuvip.com */
                ecs_header("Location: flow.php?step=checkout\n");
				/* 四合一修改 end by www.hongyuvip.com */
                exit;
            }
            else
            {
...
elseif ($_REQUEST['step'] == 'checkout')
{
   ...
   // else if($_SESSION['extension_code'] == 'virtual_good')
    ...
    {
        /* 如果不完整则转向到收货人信息填写界面 */
		/* 代码修改_start  By  www.hongyuvip.com  注释或删掉  */
        //ecs_header("Location: flow.php?step=consignee\n");
        //exit;
...
elseif ($_REQUEST['step'] == 'select_payment')
{
    ...
    else
    {
		...
		/*余额额支付密码_添加_START_www.hongyuvip.com*/
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
	    ...
	    $order = array(
	        ...
	        /*增值税发票_删除_START_www.hongyuvip.com*/
        	//'inv_type'        => $_POST['inv_type'],
        	//'inv_payee'       => trim($_POST['inv_payee']),
        	//'inv_content'     => $_POST['inv_content'],
			/*增值税发票_删除_END_www.hongyuvip.com*/
    ...
    foreach($order_info as $ok=>$order){
	    ...
	    /* 插入订单商品 下面这个SQL有修改 by www.hongyuvip.com 注意末尾那个字段 */
	    /* 代码增加_start  By www.hongyuvip.com */
	...
	//$GLOBALS['db'] -> query("unlock tables");
	    ...
	    if ($order['order_amount'] <= 0)
	    {
	        ...
	        foreach ($res AS $row)
	        {
                    /* 代码增加_start  By www.hongyuvip.com _sunlizhi*/
...
//                    }
                    ...
                    /* 代码增加_end  By www.hongyuvip.com _sunlizhi*/
...
elseif ($_REQUEST['step'] == 'add_package_to_cart')
{
    ...
    {
        /* 添加到购物车 修改 by www.hongyuvip.com 增加两个参数 */
...
{
    /* 查询：购物车中所有不可以单独销售的配件 */
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";//添加 www.hongyuvip.com
