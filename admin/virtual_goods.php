...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
   ...
   foreach($district_list_ids_arr as $k=>$v){
	...
	if($goods_number>0)
	{
        ...
        $msg_goods_url = build_uri('goods', array('gid'=>$msg_goods_id), $msg_goods_name);
        $sms_content   = array($GLOBALS['_CFG']['sms_goods_stockout_tpl'], "{\"msgGoodsName\":\"$msg_goods_name\",\"msgGoodsUrl\":\"$msgGoodsUrl\"}",$GLOBALS['_CFG']['sms_sign']);
		...
		if($list)
		{
			for($i = 0;$i < count($list); $i++)
			{
				...
				if($GLOBALS['_CFG']['sms_pricecut'] == 1)
				{
					//降价通知短信内容
                    $goods_url = build_uri('goods', array('gid'=>$goods_id), $goods_name);
                    $pricecut_content = array($GLOBALS['_CFG']['sms_pricecut_tpl'], "{\"goodsName\":\"$goods_name\",\"goodsUrl\":\"$goods_url\"}",$GLOBALS['_CFG']['sms_sign']);
					if($list[$i]['mobile'] != '')
					{
